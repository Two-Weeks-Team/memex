...
async fn client() -> Qdrant {
    ...
    // create_collection: under `cargo test`'s default parallelism these heavy
    // (multivector + sparse) collections are created concurrently, and the client's
    // default deadline can fire before the server reports the collection ready.
    Qdrant::from_url(&url)
        .timeout(60u64)
        .build()
